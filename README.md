...
📝 Edit your frontend `App.jsx` in `react-app/src`
...
📲 Spend some time in `App.jsx` in `react-app/src` and learn about the 🛰 [Providers](https://github.com/austintgriffith/scaffold-eth#-web3-providers)
...
🛰 Ready to deploy to a testnet? Change the `defaultNetwork` in `hardhat/hardhat.config.js`
...
📝 Edit your frontend `App.jsx` in `react-app/src` and open http://localhost:3000
...
📝 Edit your smart contract `SmartContractWallet.sol` in `hardhat/contracts`

> 🤡 There is a spelling error in `hardhat/contracts/SmartContractWallet.sol`! <br/><br/>🤔 Can you fix it and deploy the contract locally?

![Deployed Contract](https://user-images.githubusercontent.com/2653167/81483187-8146b380-91f9-11ea-80f0-3a8e1e3225dd.png)

> ☢️ **Warning**: It is very important that you find `SmartContractWallet.sol` in `hardhat/contracts` because there are other contract folders and it can get confusing.

🔬Test your contracts by editing `myTest.js` in `hardhat/contracts`:
...
> 🔧 Configure 👷[HardHat](https://hardhat.org/config/) by editing `hardhat.config.js` in `hardhat`
...
🔏 Edit or rename your smart contract `YourContract.sol` in `hardhat/contracts`
...
Commonly used Ethereum hooks located in `react-app/src/`:
...
Your commonly used React Ethereum components located in `react-app/src/`:
...
📝 Then edit the `SmartContractWallet.js` React component in `react-app/src`
